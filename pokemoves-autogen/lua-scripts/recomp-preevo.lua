...
for poke, data in pairs(pokemoves) do
    for gen = 1, CURRGEN do
        if
            not lib.getNdex(poke)
            and evodata[poke]
            and poke ~= evodata[poke].name -- Isn't a base form
        then
            -- Compute preevo: iterate over all moves that preevo can learn
            local preevos =
                tab.map(evolib.preevoList(poke), forms.uselessToEmpty)
            local res = {}
            for _, preevo in pairs(preevos) do
                for _, move in
                    pairs(lib.learnset(preevo, gen, { "preevo", "tutor" }))
                do
                    if
                        not lib.canLearn(move, poke, gen, { "preevo", "event" })
                    then
                        if res[move] then
                    ...
                    then
                        ...
                        if res[move] then
                            if res[move].games then
                                if
                                    not tab.equal(res[move].games, moveGames)
                                then
                                    res[move].games = nil
                                else
                                    table.insert(res[move], pokes[preevo].ndex)
