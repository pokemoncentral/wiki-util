...
# -*- coding: utf-8 -*-
r"""
This bot replaces learnlists with the output of a lua script.
...
The bot works on pages specified by a generator, such as the category of
learnset subpages.
...
[[en:{poke} (Pokémon)/Generation {gen_roman} learnset]]
    ...
    def get_noinclude_cats(self):
        ...
        return SectionsBot.NOINCLUDE_CATS.format(
            gen_text=self.generation["text"], gen_roman=self.generation["roman"]
        )
    ...
    def treat_page(self):
        ...
        # Parsing the page
        content = mwparser.parse(self.current_page.text, skip_style_tags=True)
        ...
        try:
            ...
            noinclude_cats = mwparser.parse(
                self.get_noinclude_cats().format(poke=roottitle)
            )
        ...
        for heading, kind in SectionsBot.SECTIONS:
            for section in limited_content.get_sections(matches=heading):
                ...
                newcontent = subprocess.run(
                    [
                        "lua",
                        self.command_path,
                        roottitle,
                        kind,
                        str(self.generation["number"]),
                    ],
                    capture_output=True,
                    encoding="utf-8",
                )
        ...
        self.put_current(str(content), summary=self.summary)
